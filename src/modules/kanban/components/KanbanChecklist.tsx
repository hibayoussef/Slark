...
import type { Card, Checklist } from '../models/kanban';
...
const KanbanChecklist: FC<KanbanChecklistProps> = (props) => {
  ...
  const { deleteChecklist, updateChecklist } = useKanban(state => state);
  ...
  const handleNameSave = async (): Promise<void> => {
    try {
      ...
      setEditingName(false);
      await updateChecklist(card.id, checklist.id, { name });
  ...
  const handleDelete = async (): Promise<void> => {
    try {
      await deleteChecklist(card.id, checklist.id);
